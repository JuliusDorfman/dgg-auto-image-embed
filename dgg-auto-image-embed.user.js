...
(function(){
    ...
    // femboy.beauty and kappa.lol both use the same uploader spec:
    // https://DOMAIN/<id>, https://DOMAIN/<id>.png, https://DOMAIN/<id>/name.png
    // all resolve to the same file — no scraping needed, fetch directly.
    var femboyBeautyPageRegex = /^https?:\/\/(?:[^.]+\.)?femboy\.beauty\/[A-Za-z0-9]+(?:\/[^\s"'<>]*)?(?:\?[^\s"'<>]*)?$/i;
    var kappalolPageRegex = /^https?:\/\/(?:[^.]+\.)?kappa\.lol\/[A-Za-z0-9]+(?:\/[^\s"'<>]*)?(?:\?[^\s"'<>]*)?$/i;
    ...
    // Both femboy.beauty and kappa.lol implement the same uploader spec:
    // the server responds with the raw file at /id, /id.ext, or /id/anything.ext —
    // so we can fetch the URL directly without any HTML scraping.
    function isFemboyBeautyPage(u){ return femboyBeautyPageRegex.test(u); }
    function isKappaLolPage(u){ return kappalolPageRegex.test(u); }
    ...
    function handleLink(el){
        ...
        // femboy.beauty and kappa.lol both use the uploader spec — the URL itself IS the image.
        // Fetch it directly; the server returns raw file bytes for any /<id>[/anything][.ext] path.
        if (isFemboyBeautyPage(el.href) || isKappaLolPage(el.href)){
            fetchAndAppend(el, el.href, el.href);
            return;
        }
